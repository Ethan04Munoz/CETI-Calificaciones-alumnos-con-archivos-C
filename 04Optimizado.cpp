...
{
    vector<int>registros; //Añadimos vectores para que puedan ser llenados más adelante la cantidad necesaria solamente
...
int main() { //Ethan Muñoz López          19300098                3C         21/10/2020
    ...
    cin >> verPasaron;
    //Impresion del formato //Generacion de archivo
    ofstream archivo; //Escribir
    archivo.open("C:\\Users\\Ethan\\Desktop\\calificaciones.txt", ios::out); //Creamos el archivo y lo abrimos en modo escritura Aqui se debe remplazar la ubicacion del archivo
    printf("Registro escolar de un grupo\n\n____________________________________________________________\n\n");
    archivo << "Registro escolar de un grupo\n\n____________________________________________________________\n\n";
    for (int i = 0; i < cantidadEstudiantes; i++) {
        if (i == 0) {
            ...
            for (int j = 0; j < listaAlumnos.materias.size(); j++) {
                ...
                archivo << listaAlumnos.materias[j] << "          ";
            }
            printf("\n\n");
            archivo << "\n\n";
        }
        archivo << listaAlumnos.registros[i] << "   " << listaAlumnos.nombres[i] << "   "; //Escribimos los datos de los alumnos
        archivo << listaAlumnos.apellido1[i] << "   " << listaAlumnos.apellido2[i] << "   ";
        archivo << "            ";
        ...
        for (int j = 0; j < cantidadMaterias; j++) {
            ...
            archivo << arregloCalificaciones[i][j] << "          "; //Escribimos en el archivo las calificaciones
        }
        printf("\n");
        archivo << "\n";
    }
    printf("\n\n");     //Añadimos las opciones que se pueden elegir al archivo también
    archivo << "\n\n";
    if (verAprobacion == "si") { //En el caso de que el usuario nos haya dicho que si quiere ver los porcentajes añadimos esta opcion
        for (int j = 0; j < cantidadMaterias; j++) {
            ...
            archivo << "El porcentaje de aprobacion de " << listaAlumnos.materias[j] << " es ";
            ...
            archivo << porcentajeAprobacion << "%\n";
            cout << porcentajeAprobacion << "%\n";
            float porcentajeReprobados = 0; //Verificar Reprobacion
            ...
            archivo << "El porcentaje de reprobacion de " << listaAlumnos.materias[j] << " es ";
            archivo << porcentajeReprobados << "%\n";
        }
    }
    ...
    if (verPasaron == "si") { //En el caso de que el usuario nos haya dicho que si quiere ver los aprobados añadimos esta opcion
        for (int j = 0; j < cantidadMaterias; j++) {
            ...
            archivo << "\n\n-----------\n";
            archivo << "Aprobaron la materia " << listaAlumnos.materias[j] << "\n";
            for (int i = 0; i < cantidadEstudiantes; i++) {
                if (arregloCalificaciones[i][j] >= 70) {
                    ...
                    archivo << listaAlumnos.nombres[i] << " " << listaAlumnos.apellido1[i] << " " << listaAlumnos.apellido2[i] << " con calificacion de " << arregloCalificaciones[i][j] << "\n";
                }
            }
        }
    }
    printf("\n\n________________________________________\n\nGenial! Ten un buen dia\n\nEspera! Estamos generando tu archivo de texto con las calificaciones!...\n\nEsta listo!\n\n\n");
